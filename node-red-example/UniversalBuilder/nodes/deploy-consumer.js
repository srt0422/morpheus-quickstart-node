...
module.exports = function(RED) {
    ...
    async function deployConsumer(node, msg) {
        ...
        // Check for required proxy URL
        if (!msg.config.proxyUrl) {
            msg.payload = {
                status: 'error',
                action: 'deploy',
                error: 'Missing proxy URL from previous deployment step',
                proxyUrl: null,
                consumerUrl: null
            };
            node.send(msg);
        ...
        try {
            const { dockerRegistry, consumerImage, region, projectId } = {
                dockerRegistry: msg.config.dockerRegistry || env.DOCKER_REGISTRY,
                consumerImage: msg.config.consumerImage || 'openai-morpheus-consumer:latest',
                region: msg.config.region || env.REGION,
                projectId: msg.config.projectId || env.PROJECT_ID
            };
            const proxyUrl = msg.config.proxyUrl;
